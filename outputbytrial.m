function[Q_rl,Q_wm,loss,Q_rl_bytrial,Q_wm_bytrial]=outputbytrial(keytable,para)
...
loss=0;
Q_rl_bytrial=ones(length(stimulus),length(unique(stimulus)),num_choice).*Q_0;
Q_wm_bytrial=ones(length(stimulus),length(unique(stimulus)),num_choice).*Q_0;
